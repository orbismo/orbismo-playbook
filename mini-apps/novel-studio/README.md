...
| Entity | Role |
| --- | --- |
| `rule/app_novel_studio` | The app itself. Nine lore chunks carry the full spec: Data Model: Novels & the Shelf; Characters; Scenes; Threads & Ideas; Canon Rules; Packs; Workflow: Develop the Story; Plot Scenes & Threads; Review & Continuity. |
...
| [Sci-Fi](packs/sci-fi/) | Genre | Technology and anomaly threads; tech-consistency, Chekhov's-tech, extrapolation, and timeline lenses backed by canon rules. |
...
- Entity types `project`, `person`, `event`, `reference`, `interest`, and `rule` (Orbismo's common types).
...
- **The world's laws are canon rules.** "In this world, magic costs memory" becomes a `rule` entity (`rule_type: "canon"`) listed in the novel's index; the continuity lint then flags any scene that breaks it. Laws are recorded only when you state them — the assistant never legislates your world unasked.
